...
% take the pointwise power with a gen posynomial and a scalar (to get a gposy)
if( length(obj1) == 1 & length(obj2) == 1 )
  r = gposynomial(obj2, {obj1});
  return;
end

% pointwise power of a gen posynomial vector (or matrix) and scalar power
if( length(obj2) == 1 )
  for i = 1:sz1(1)
    for j = 1:sz1(2)
      r(i,j) = obj1(i,j)^obj2;
...
% pointwise power of a gen posynomial vector and a scalar vector
if( sz1(1) == 1 & sz2(1) == 1 )
  for k = 1:sz1(2)
...
% pointwise power of matrices (one has gen posynomials and the other scalars)
for i = 1:sz1(1)
  for j = 1:sz1(2)
